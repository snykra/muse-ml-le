# https://github.com/Timm877/BCI_Project/blob/main/util/VisualizeDataset.py
...
class VisualizeDataset:
    ...
    def __init__(self, module_path='.py'):
        ...
        self.plot_number = 1
    def save(self, plot_obj, graph_type, filename, formats=('png',)): # 'svg'
        fig_name = f'{graph_type}_{filename}_{self.plot_number}'
    ...
    def plot_dataset(self, data_table, columns, match, display, algo = 'x', filename='test'):
        names = list(data_table.columns)

        # Create subplots if more columns are specified.
        if len(columns) > 1:
            f, xar = plt.subplots(len(columns), sharex=True, sharey=False, figsize=(25,25))
        else:
            f, xar = plt.subplots()
            xar = [xar]
        ...
        for i in range(0, len(columns)):
            ...
            xar[i].legend(relevant_cols, fontsize='small', numpoints=1, loc='upper center',
                          bbox_to_anchor=(0.5, 1.3), ncol=len(relevant_cols), fancybox=True, shadow=True, prop={'size': 12})
        ...
        plt.xlabel('time')
        self.save(plt, 'plot_dataset', filename + '_' + algo)

    def plot_xy(self, x, y, method='plot', xlabel=None, ylabel=None, xlim=None, ylim=None, names=None,
                line_styles=None, loc=None, title=None, filename='test', algo='test'):
        for input in x, y:
        ...
        self.save(plt, 'plot_xy', filename + '_' + algo)
        plt.show()

    def plot_dataset_boxplot(self, dataset, cols, filename):
        plt.Figure(); dataset[cols].plot.box()
        plt.ylim([-3,3])
        self.save(plt, 'BOXPLOT', filename)
    ...
    # This function plots the predictions of an algorithms (both for the training and test set) versus the real values for
